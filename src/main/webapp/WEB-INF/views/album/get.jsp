				...
				<div>
					<c:choose>
						<c:when test="${move.next != 9999 && move.next != 0}">					
							<a href="/album/get?ano=${move.next}"> <i class= "fa fa-arrow-circle-left"></i> 다음글: ${move.nexttitle}</a>
						</c:when>				
				...
				<div>				
					<c:choose>
						<c:when test="${move.prev != 9999 && move.prev != 0}">						
							<a href="/album/get?ano=${move.prev}"> 이전글: ${move.prevtitle} <i class= "fa fa-arrow-circle-right"></i></a>
						</c:when>					
